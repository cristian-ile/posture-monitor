        ...
        while not self.stop_flag.is_set():
            try:
                if USE_WINSOUND:
                    ...
                    tone = (np.sin(self.frequency * t * 2 * np.pi) * 0.35)
...
# ---------------- Alert Manager ----------------
class AlertManager:
    def __init__(self, root, title, message, beep_frequency=880):
        self.root = root
        self.popup = None
        self.sound = SoundPlayer()
        self.sound.frequency = beep_frequency
        self.active = False
        self.title = title
...
class PostureApp:
    def __init__(self, root):
        ...
        self.posture_alert = AlertManager(root, "Posture Warning", "ADJUST POSTURE PLEASE!", beep_frequency=880)
        self.move_alert = AlertManager(root, "Movement Reminder", "TIME TO MOVE! STAND UP AND STRETCH", beep_frequency=760)
        self.microbreak_alert = AlertManager(root, "Microbreak Reminder", "TAKE A SHORT MICROBREAK: MOVE OR STRETCH", beep_frequency=700)
